...
module MakeInterval (Endpoint : ORDERED_TYPE) = 
  struct
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      ...
      create (if Endpoint.compare l1 l2 > 0 then l1 else l2)
             (if Endpoint.compare h1 h2 < 0 then h1 else h2)
    end ;;
